// eslint-disable-next-line no-unused-vars
import React, { useState, useEffect } from 'react';

import { FlowConnection } from '@carbon/icons-react';
import { getName } from '../../utils/elementUtil';
import Tree from './Tree';
import { VariableOrigin, VariableType } from './Labels';

export default function(
    {
      variables,
      variableMap = {},
      injector = { get: () => { } },
      layout = {},
      setLayout = () => { }
    }
) {

  console.log(variables, variableMap);

  // Highlight handling
  const [ highlightedElements, setHighlightedElements ] = useState([]);

  const onSelect = (ids) => {
    if (!ids) {
      return setHighlightedElements([]);
    }

    if (!Array.isArray(ids)) {
      ids = [ ids ];
    }

    setHighlightedElements(ids);
  };

  useEffect(() => {
    const canvas = injector.get('canvas');
    const elementRegistry = injector.get('elementRegistry');

    const existingElements = highlightedElements.filter(id => elementRegistry.get(id));

    existingElements.forEach(id => canvas.addMarker(id, 'dataOutline-highlight'));

    return () => {
      existingElements.forEach(id => canvas.removeMarker(id, 'dataOutline-highlight'));
    };
  }, [ injector, highlightedElements ]);


  // Skip root level - it's always definitons and not relevant for the user
  const variableNodes = Object.keys(variableMap).flatMap(key => {
    return createNodes({
      variables,
      variableMap: variableMap[key],
      idPrefix: key,
      onSelect
    });
  });

  return <Tree variableNodes={ variableNodes } layout={ layout } setLayout={ setLayout } />;
}

const createNodes = ({
  key,
  variables,
  variableMap,
  idPrefix = '',
  onSelect
}) => {
  const childNodes = Object.keys(variableMap).flatMap(key => {
    if (key.startsWith('$')) {
      return [];
    }

    // Handle nested scopes
    const scope = variableMap[key];
    const id = idPrefix + '-' + key;

    const onMouseEnter = (e) => {
      onSelect(key);
    };

    const onMouseOut = (e) => {
      if (!e.currentTarget.contains(e.relatedTarget)) {
        onSelect([]);
      }
    };

    return {
      id: id,
      value: id,
      onMouseEnter,
      onMouseOut,
      label: getName(scope.$details),
      isExpanded: true,
      renderIcon: FlowConnection,
      children: createNodes({ variables, variableMap: scope, idPrefix: id, onSelect, key })
    };
  });

  const nodes = [];

  const calculatedVariables = createVariableNodes({
    variables: variableMap['$variables'],
    onSelect,
    idPrefix
  });

  const usedVariables = variables.filter(v => {
    const origin = v.origin.find(o => o.id === key);
    if (!origin) {
      return;
    }

    console.log(origin, v.scope, v.scope !== origin);
    return origin && v.scope.id !== origin.id;
  });

  console.log(usedVariables, variables, key);

  const variableNodes = calculatedVariables ? [
    {
      id: idPrefix + 'written',
      label: 'Written Variables',
      isExpanded: true,
      children: createVariableNodes({
        variables: usedVariables,
        onSelect,
        idPrefix
      })
    },
    {
      id: idPrefix + 'read',
      label: 'Used Variables',
      isExpanded: false
    },
    {
      id: idPrefix + 'available',
      label: 'Available Variables',
      isExpanded: false,
      children: calculatedVariables
    }
  ] : [];

  calculatedVariables && nodes.push(...variableNodes);

  childNodes && nodes.push(...childNodes);

  return nodes;
};

function createVariableNodes({ variables, idPrefix = '', onSelect }) {
  if (!variables || !variables.length) {
    return null;
  }

  return variables.map(variable => {

    const onMouseEnter = (e) => {
      onSelect(variable.origin?.map(o => o.id));
    };

    const onMouseOut = (e) => {
      if (!e.currentTarget.contains(e.relatedTarget)) {
        onSelect([]);
      }
    };

    return {
      id: idPrefix + '-' + variable.name,
      value: variable.name,
      label: <>
        {variable.name}
        <VariableOrigin variable={ variable } />
        <VariableType variable={ variable } />
      </>,
      onMouseEnter: variable.origin?.length && onMouseEnter,
      onMouseOut: variable.origin?.length && onMouseOut,
      isExpanded: false,
      children: createVariableNodes({ variables: variable.entries, idPrefix: idPrefix + '-' + variable.name, onSelect })
    };
  });
}

